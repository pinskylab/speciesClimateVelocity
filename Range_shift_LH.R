...
# Latitude through time for each species in each region in many graphs
	...
	for(k in 1:length(regs)){ # for each region
		indreg = maxlat$region == regs[k] & (as.character(maxlat$spp) %in% as.character(spppres1$taxon[spppres1$use])) # show only species resolves at least to genus (spppres1$use)
...
# Rate of lat/lon/biomass shift calculations (by regspp)
	...
	for(i in 1:nrow(centbiolatindiv)){
		...
		for(j in 1:length(vars)){
			nm = names(vars)[j]
			lcols = grep('X', names(vars[[j]])) # columns with latitude or other data
...
#	shiftcols = cols[7]
	...
	map2(database='state', add=TRUE, fill=TRUE, col=bcol, bcol='white')
