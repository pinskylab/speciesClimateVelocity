## How fast are species moving in AFSC Gulf of Alaska data?
...
data1 = read.csv('Processed/goa1984_1987.csv')
	dim(data1)
	names(data1)
data2 = read.csv('Processed/goa1990_1999.csv')
	dim(data2)
	names(data2)
data3 = read.csv('Processed/goa2001_2005.csv')
	dim(data3)
	names(data3)
data4 = read.csv('Processed/goa2007_2011.csv')
	dim(data4)
	names(data4)

data = rbind(data1, data2, data3, data4)
	dim(data) # 184458 rows
...
# Create a unique haulid
	...
	length(unique(data$haulid)) # 9213

# Trim to high quality strata (sampled every year)
	i = table(data$STRATUM, data$YEAR); i
		all(rowSums(i>0)==11)
	data = data[!(data$STRATUM %in% c(50, 210, 410, 420, 430, 440, 450, 510, 520, 530, 540, 550)),] # strata to remove.
		dim(data)

# Trim to high-quality years (sample all strata)	
	data = data[!(data$YEAR %in% 2001),] # 2001 didn't sample many strata
		dim(data) #162513
	
# Reasonable depths? 
#	hist(data$BOT_DEPTH) # 
	range(data$BOT_DEPTH, na.rm=T) # 13-530

	# Plot a map of survey points colored by depth
		# set up the colors
#		d = round(data$BOT_DEPTH/10)*10
#		d = rank(d, na.last='keep', ties.method='min')
#		du = sort(unique(d))
#		for(i in 1:length(du)) d[d==du[i]] = i
#		cols = terrain.colors(length(du))
#	plot(data$LONGITUDE, data$LATITUDE, pch=16, col=cols[d])
	
	table(data$STRATUM, data$YEAR)
...
# What months,days,times?
	...
	table(data$month, data$year)
		# was June-October in 1984
		# was May-September in 1987
		# was June-September in 1990, 1993
		# was May-July in 1996, 1999, 2009
		# was May-August in 2003, 2005, 2007, 2011
...
# Add strata areas
	...
	sort(setdiff(unique(data$stratum), unique(strata$stratum)))
	dim(data)
...
# Remove spp without a scientific name
	...
	data = data[!i,]
	dim(data) # 156784 rows
	length(unique(data$haulid)) # 8242
...
# Find duplicate tows (repeat rows for the same tow), and combine entries from spp that have now been consolidated
	...
	for(i in 1:length(inds)){
		...
		# inds2 = which(data$sppold == data$sppold[inds[i]] & data$year == data$year[inds[i]] & data$lat == data$lat[inds[i]] & data$lon == data$lon[inds[i]])
...
# How many tows?
	length(unique(paste(newdata$year, newdata$lat, newdata$lon))) # 8242 unique locations
	length(unique(newdata$haulid)) # 8242: good that this matches # locations


# How many spp?
	length(unique(newdata$spp)) #1188

# Add a region column
newdata$region = "AFSC_GOA"
...
# Identify the spp to analyze
	...
	nrow(spplist) # 1188 spp
	
	sum(spplist$pres>1) # 928
	sum(spplist$pres>10) # 512
	max(spplist$presyr) # 11 yrs of data
	sum(spplist$presyr == max(spplist$presyr)) # 126 spp present every year

	spplist = spplist[spplist$presyr == max(spplist$presyr),] # take all spp present >= 1x per yr
		nrow(spplist) # 126 spp	
...
# only keep spp that are caught > 10 times/yr
	...
	sppnames # These are spp to consider using: 58

	sppnames = sppnames[!(sppnames %in% c('gastropodeggs', 'Scyphozoa'))] # remove some spp
	length(sppnames) # 57
	
	# Trim data to right spp
	datatrim = data[data$spp %in% sppnames,]
	dim(datatrim) # 469,794 x 24
