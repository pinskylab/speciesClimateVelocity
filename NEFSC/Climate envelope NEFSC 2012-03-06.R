...
## Figure out which names we need to use and trim the data
	...
	i = order(yr)
	table(yr[i], dataspp$spp_name[i])

	# Table of when each name was used and has abundance > 0
	yr = as.numeric(substr(dataspp$startdateGMT,9,12))
	j = dataspp$expcatnum > 0 | dataspp$cpue > 0
	table(yr[j], dataspp$spp_name[j])
...
# Save temp data
save(btemp, btemp2, stemp, stemp2, yr, file=paste('Output/Temp_IOOS_', season, '_', Sys.Date(), '.RData', sep=''))
