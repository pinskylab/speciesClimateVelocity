...
data = read.csv('Output/datatrimwzeros_2012-03-05.csv', row.names=1)
	dim(data)
	
## Figure out which names we need to use and trim the data
# only keep spp that are caught > 10 times/yr
	tab = aggregate(data$biomass>0, by=list(spp=data$spp, year = data$year), FUN=sum, na.rm=T)
	...
	sppnames # These are spp to consider using
...
## Add HadISST data (min and max temp)
	...
	datatrim$longrid = floor(datatrim$longitude)+0.5
...
# Transform data
	datatrim$lsurftemp = log(datatrim$t_surface+1)
	datatrim$lbottemp = log(datatrim$t_bottom+1)
	datatrim$lmintemp = log(datatrim$mintemp+1)	
	
# Add pres/abs
	datatrim$pres = datatrim$catch>0 | (is.na(datatrim$catch) & !is.na(datatrim$biomass) & datatrim$biomass>0)

	sum(is.na(datatrim$pres)) # should be 0

	m = which(is.na(datatrim$pres))
	datatrim[m,] # one case

	datatrim$pres[m] = FALSE # turn NA to FALSE (catch is NA, biomass == 0)

## Add mean biomass as a predictor
bm = aggregate(list(biomassmean=datatrim$biomass), by=list(year=datatrim$year, spp=datatrim$spp), FUN=mean, na.rm=T)
	dim(bm)
	datatrim = merge(datatrim, bm)
	dim(datatrim)

bm = aggregate(list(nummean=datatrim$catch), by=list(year=datatrim$year, spp=datatrim$spp), FUN=mean, na.rm=T)
	dim(bm)
	datatrim = merge(datatrim, bm, all.x=T)
	dim(datatrim)

# Have a cpue that never goes to zero (useful for fitting log-links)
datatrim$wtcpuena = datatrim$biomass
datatrim$wtcpuena[datatrim$wtcpuena == 0] = 1e-4
datatrim$wtcpuenal = log(datatrim$wtcpuena)

datatrim$numcpuena = datatrim$catch
datatrim$numcpuena[datatrim$numcpuena == 0] = 1e-4
datatrim$numcpuenal = log(datatrim$numcpuena)

# Add a region column
datatrim$region = "DFO_SoGulf"

# Add blank salinity and station columns
datatrim$surfsal = NA
datatrim$station = NA

# Rename columns
names(datatrim)[names(datatrim)=='vessel'] = 'svvessel'
names(datatrim)[names(datatrim)=='strat'] = 'stratum'
names(datatrim)[names(datatrim)=='set'] = 'tow'
names(datatrim)[names(datatrim)=='latitude'] = 'lat'
names(datatrim)[names(datatrim)=='longitude'] = 'lon' # use the adjusted longitude
names(datatrim)[names(datatrim)=='t_surface'] = 'surftemp'
names(datatrim)[names(datatrim)=='t_bottom'] = 'bottemp'
names(datatrim)[names(datatrim)=='biomass'] = 'wtcpue'
names(datatrim)[names(datatrim)=='catch'] = 'numcpue'
names(datatrim)[names(datatrim)=='salin_bottom'] = 'botsal'
...
datatrim = read.csv('Output/dataCEM_2012-03-05.csv', row.names=1)
